...
class DifyPluginEnv(BaseSettings):
    ...
    REMOTE_INSTALL_URL: str | None = Field(
        default=None, description="Remote installation URL"
    )
    ...
    REMOTE_INSTALL_KEY: str | None = Field(
    ...
    SERVERLESS_WORKER_CONNECTIONS: int = Field(
        default=1000, description="Serverless worker connections"
    )
