...
class DatasourceActions(StrEnum):
    ...
    InvokeOnlineDocumentDatasourceGetPages = (
        "invoke_online_document_datasource_get_pages"
    )
    InvokeOnlineDocumentDatasourceGetPageContent = (
        "invoke_online_document_datasource_get_page_content"
    )
...
class DatasourceGetPageContentRequest(PluginAccessRequest):
    ...
    action: DatasourceActions = (
        DatasourceActions.InvokeOnlineDocumentDatasourceGetPageContent
    )
    provider: str
    datasource: str
    credentials: Mapping[str, Any]
...
class TriggerInvokeEventRequest(BaseModel):
    ...
    payload: Mapping[str, Any] = Field(
        ...
        description=(
            "The decoded payload from the webhook request, which will be "
...
class TriggerDispatchResponse(BaseModel):
    user_id: str = Field(
        description="The user who triggered the event (e.g. google user ID)"
    )
