...
class InvokeCredentials(BaseModel):
    ...
    Session Level Credentials, used to store session level credentials, such
    as tool call credentials.
    Especially for the backwards invoke, when invoker is not specified, we
    need to use the credential id from the
    session context.
    """
    ...
    tool_credentials: dict[str, str] = Field(
        ...
        description=(
            "This is a map of tool provider to credential id. It is used to "
            "store the credential id for the tool provider."
        ),
    )
...
class SessionContext(BaseModel):
    ...
    In the future, we will refactor message_id and conversation_id and the
    others to be part of the session context.
    """
...
class Session:
    def __init__(
        ...
        self.context: SessionContext = (
            SessionContext.model_validate(context)
            if isinstance(context, dict)
            else context or SessionContext()
        )
...
class BackwardsInvocation[T: BaseModel | dict | str]:
    ...
    def _line_converter_wrapper(
        ...
        # get max timeout count, each wait is 1 second, so timeout count
        # equals timeout seconds
