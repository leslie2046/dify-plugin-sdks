...
class ToolLike[T: InvokeMessage](ABC):
    ...
    def create_log_message(
        ...
        status: InvokeMessage.LogMessage.LogStatus = (
            InvokeMessage.LogMessage.LogStatus.SUCCESS
        ),
        parent: T | None = None,
    ...
    @deprecated(
        "This feature is deprecated, will soon be replaced by dynamic select parameter"
    )
...
class ToolProvider:
    ...
    def oauth_get_credentials(
        ...
        return OAuthCredentials(
            credentials=tool_oauth_credentials.credentials,
            expires_at=tool_oauth_credentials.expires_at,
        )
