...
class MultiModalContent(BaseModel):
    ...
    content: str = Field(
        ..., description="The payload content, plain text or base64 encoded file data."
    )
    content_type: MultiModalContentType = Field(
        ..., description="The modality of the provided content."
    )
...
class MultiModalEmbeddingResult(BaseModel):
    ...
    model: str = Field(
        ..., description="Identifier of the model generating embeddings."
    )
    embeddings: list[list[float]] = Field(
        ..., description="Embedding vectors for provided contents."
    )
    usage: EmbeddingUsage = Field(
        ..., description="Usage metrics associated with the inference."
    )
...
class MultiModalEmbeddingModelConfig(BaseModelConfig):
    ...
    model_type: ModelType = ModelType.TEXT_EMBEDDING
    tenant_id: str = Field(
        ..., description="Vendor tenant identifier associated with the dataset."
    )
