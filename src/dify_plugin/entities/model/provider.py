...
class ModelPosition(BaseModel):
    ...
    llm: list[str] | None = Field(
        ...
        description="Sorts of llm model in ascending order, fill model name here",
    )
    text_embedding: list[str] | None = Field(
        default_factory=list,
        description=(
            "Sorts of text embedding model in ascending order, fill model name here"
        ),
    )
    rerank: list[str] | None = Field(
        default_factory=list,
        description="Sorts of rerank model in ascending order, fill model name here",
    )
    tts: list[str] | None = Field(
        default_factory=list,
        description="Sorts of tts model in ascending order, fill model name here",
    )
    speech2text: list[str] | None = Field(
        default_factory=list,
        description=(
            "Sorts of speech2text model in ascending order, fill model name here"
        ),
    )
    moderation: list[str] | None = Field(
        default_factory=list,
        description=(
            "Sorts of moderation model in ascending order, fill model name here"
        ),
    )
