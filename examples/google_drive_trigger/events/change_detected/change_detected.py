...
class GoogleDriveChangeDetectedEvent(Event):
    ...
    def _on_event(
        ...
        file_name_patterns = self._normalize_string_list(
            parameters.get("file_name_pattern")
        )
        ...
        variables = {
            ...
            "subscription": {
                ...
                "watch_expiration": self.runtime.subscription.properties.get(
                    "watch_expiration"
                ),
    ...
    def _filter_changes(
        ...
        *,
        ...
        allowed_change_types = {
            change_type.lower() for change_type in change_types if change_type
        }
        ...
        for change in changes:
            ...
            normalized_change_type = (
                str(change_type_value).lower() if change_type_value is not None else ""
            )
            if (
                allowed_change_types
                and normalized_change_type not in allowed_change_types
            ):
                continue
            ...
            if normalized_patterns and (
                not file_name
                or not any(
                    fnmatchcase(file_name, pattern) for pattern in normalized_patterns
                )
            ):
                continue
