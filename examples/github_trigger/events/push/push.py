...
class PushEvent(Event):
    ...
    def _on_event(
        ...
        self._check_commit_message_contains(
            payload, parameters.get("commit_message_contains")
        )
    ...
    def _check_branch(
        ...
        allowed_branches = [
            branch.strip() for branch in branch_param.split(",") if branch.strip()
        ]
    ...
    def _check_pusher(
        ...
        allowed_pushers = [
            pusher.strip() for pusher in pusher_param.split(",") if pusher.strip()
        ]
