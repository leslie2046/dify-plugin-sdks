...
class VcLeaveMeetingV1Event(Event):
    def _on_event(
        ...
        variables_dict: dict[str, Any] = {
            "leave_reason": event_data.leave_reason or "",
        }

        # Add leave user information
        if event_data.leave_user:
            variables_dict.update({
                "leave_user_id": event_data.leave_user.id or "",
                "leave_user_type": str(event_data.leave_user.user_type)
                if event_data.leave_user.user_type
                else "",
            })
