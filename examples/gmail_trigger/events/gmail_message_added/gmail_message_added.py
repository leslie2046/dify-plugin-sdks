...
class GmailMessageAddedEvent(Event):
    ...
    def _on_event(
        ...
        if not items:
            sub_key = (self.runtime.subscription.properties or {}).get(
                "subscription_key"
            ) or ""
        ...
        prop_label_ids: list[str] = (self.runtime.subscription.properties or {}).get(
            "label_ids"
        ) or []
        ...
        for it in items:
            ...
            existing_urls: set[str] = {
                str(meta.get("original_url"))
                for meta in attachments_meta
                if isinstance(meta.get("original_url"), str)
            }
    ...
    def _extract_external_attachment_links(
        ...
        for part in inline_parts:
            ...
            headers_list = (
                part.get("headers") if isinstance(part.get("headers"), list) else []
            )
            ...
            if mime_type == "text/html":
                ...
                candidates = [
                    (url, None) for url in self._extract_links_from_text(text_content)
                ]
            ...
            for url, label in candidates:
                ...
                if not self._is_supported_external_url(normalized_url):
                    continue
                filename = (label or "").strip() or self._derive_filename_from_url(
                    normalized_url
                )
    ...
    def _prepare_attachments(
        ...
        upload_candidates = sum(
            ...
            and (meta.get("attachmentId") or meta.get("inline_data"))
        )
        ...
        for meta in attachments:
            ...
            if (
                isinstance(size_hint, int)
                and size_hint > self._MAX_ATTACHMENT_UPLOAD_SIZE
            ):
                attachment["file_url"] = attachment.get("original_url")
                self._finalize_attachment_urls(attachment)
                processed.append(attachment)
            ...
            if content is None or (
                ...
                and actual_size > self._MAX_ATTACHMENT_UPLOAD_SIZE
            ):
                attachment["file_url"] = attachment.get("original_url")
                self._finalize_attachment_urls(attachment)
                processed.append(attachment)
    ...
    def _decode_transfer_encoding(
        content: bytes, headers: list[Mapping[str, Any]] | None
    ) -> bytes:
    ...
    def _is_supported_external_url(url: str) -> bool:
        ...
        return hostname.endswith(".google.com") and (
            "drive" in hostname or "docs" in hostname
        )
