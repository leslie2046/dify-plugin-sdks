...
class GmailTrigger(Trigger):
    ...
    def _dispatch_event(
        ...
        access_token = (
            (self.runtime.credentials or {}).get("access_token")
            if self.runtime
            else None
        )
        if not access_token:
            return EventDispatch(
                events=[],
                response=self._value_error("Missing access token for Gmail API"),
            )
        ...
        # Storage keys kept for backward compatibility in events fallback.
        # We no longer write pending batches.

        session = self.runtime.session
        if not session.storage.exist(checkpoint_key):
            # First notification: initialize checkpoint and return 200
            session.storage.set(
                checkpoint_key, str(notification["historyId"]).encode("utf-8")
            )
        ...
        messages, added, deleted, labels_added, labels_removed = (
            self._fetch_history_delta(
                headers=headers,
                user_id=user_id,
        ...
        return EventDispatch(
            events=events, response=self._ok(), payload=combined_payload
        )
    ...
    def _maybe_verify_pubsub_oidc(
        self, request: Request, props: Mapping[str, Any], endpoint: str
    ) -> None:
        ...
        self._verify_oidc_token(
            token=token, audience=audience, expected_email=expected_sa
        )
    ...
    def _verify_oidc_token(
        self, token: str, audience: str, expected_email: str | None = None
    ) -> None:
...
class GmailSubscriptionConstructor(TriggerSubscriptionConstructor):
    ...
    def _oauth_refresh_credentials(
        ...
        if credentials.get("gcp_service_account_json"):
            refreshed["gcp_service_account_json"] = credentials[
    ...
    def _create_subscription(
        ...
        # Gmail watch is time-limited; if expiration is not provided, use
        # 6 days as a safe default.
        expires_at: int = (
            int(expiration_ms / 1000)
            if expiration_ms
            else int(time.time()) + 6 * 24 * 60 * 60
        )
        ...
        if parameters.get("oidc_service_account_email"):
            properties["oidc_service_account_email"] = parameters.get(
    ...
    def _ensure_pubsub(
        ...
        # Ensure a dedicated push subscription per Dify subscription.
        ...
        try:
            subscriber.create_subscription(
                name=sub_path, topic=topic_path, push_config=push
            )
        except AlreadyExists:
    ...
    def _delete_subscription(
        ...
        if not gcp_project_id or not gcp_sa:
            return UnsubscribeResult(
                success=False, message="Missing GCP credentials for cleanup"
            )
        ...
        if not push_name:
            return UnsubscribeResult(
                success=False, message="Missing push subscription name for cleanup"
            )
        ...
        try:
            self._delete_managed_resources(
                project_id=gcp_project_id,
                sa_json=gcp_sa,
                topic_name=topic_id or "",
                subscription_name=push_name,
            )
        except Exception as exc:
        ...
        try:
            ...
            msg = (
                f"Push subscription deleted. {cleanup_msg}"
                if cleanup_msg
                else "Push subscription deleted"
            )
    ...
    def _delete_managed_resources(
        self, project_id: str, sa_json: str, topic_name: str, subscription_name: str
    ) -> None:
        """Delete dedicated Push Subscription.
        ...
        with contextlib.suppress(NotFound):
            subscriber.delete_subscription(subscription=sub_path)
        # Optionally delete topic when no managed subscriptions remain.
        # Skipped to avoid disrupting others.
