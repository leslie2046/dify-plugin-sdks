    ...
    def _system_prompt_message(self) -> SystemPromptMessage:
        ...
        system_prompt = (
            ...
            .replace(
                "{{tools}}",
                json.dumps([
                    tool.model_dump(mode="json") for tool in self._prompt_messages_tools
                ]),
            )
            .replace(
                "{{tool_names}}",
        ...
        try:
            ...
            for response in tool_invoke_responses:
                if response.type == ToolInvokeMessage.MessageType.TEXT:
                    ...
                    result += (
                        "Image has been successfully generated and saved to: "
                        f"{image_link_text}. "
                        + "The image file is now available for download. "
                        + "Please inform the user that the image has been "
