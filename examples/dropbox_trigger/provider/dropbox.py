...
class DropboxTrigger(Trigger):
    ...
    Users create their own Dropbox App and set the App's webhook URL to the
    subscription endpoint.
    This trigger validates signatures and emits a lightweight notification with
    the account IDs and raw payload.
    """
    ...
    def _dispatch_event(
        ...
        if request.method == "GET":
            ...
            if challenge:
                return EventDispatch(
                    events=[],
                    response=Response(
                        response=challenge, mimetype="text/plain", status=200
                    ),
                )
        ...
        if access_token:
            ...
            for _ in range(self._MAX_PAGES):
                page, cursor, has_more = self._list_folder_continue(
                    access_token, cursor
                )
        ...
        payload_out = {
            ...
            "raw": payload,
            "headers": {
                "x_dropbox_request_id": request.headers.get("X-Dropbox-Request-Id")
            },
            "received_at": int(time.time()),
        }

        return EventDispatch(
            events=[self._EVENT_NAME], response=self._ok_response(), payload=payload_out
        )
