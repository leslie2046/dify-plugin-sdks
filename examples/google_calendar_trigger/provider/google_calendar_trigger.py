...
class GoogleCalendarTrigger(Trigger):
    ...
    def _dispatch_event(
        ...
        calendar_id = (
            properties.get("calendar_id") or parameters.get("calendar_id") or "primary"
        )
        ...
        # The first webhook after a watch is created has resourceState=sync.
        # Nothing to process.
...
class GoogleCalendarSubscriptionConstructor(TriggerSubscriptionConstructor):
    ...
    def _refresh_subscription(
        ...
        calendar_id = (
            properties.get("calendar_id")
            or subscription.parameters.get("calendar_id")
            or "primary"
        )
        calendar_id = str(calendar_id)
