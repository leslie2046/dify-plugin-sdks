...
class TelegramSubscriptionConstructor(TriggerSubscriptionConstructor):
    ...
    def _validate_api_key(self, credentials: Mapping[str, Any]) -> None:
        ...
        if not payload.get("ok"):
            description = (
                payload.get("description") or "Telegram API rejected the token"
            )
    ...
    def _create_subscription(
        ...
        if not response_data.get("result"):
            description = (
                response_data.get("description") or "Failed to set Telegram webhook"
            )
            raise SubscriptionError(
                description,
        ...
        properties: dict[str, Any] = {
            ...
            or webhook_info.get("allowed_updates", []),
    ...
    def _delete_subscription(
        ...
        response_data = self._telegram_post(
            token, "deleteWebhook", {"drop_pending_updates": True}
        )
        if not response_data.get("ok"):
            description = (
                response_data.get("description") or "Failed to delete Telegram webhook"
            )
            raise UnsubscribeError(
                message=description,
    ...
    def _parse_subscription_response(
        ...
        if not data.get("ok"):
            description = (
                data.get("description") or f"Telegram API error during {method}"
            )
            raise SubscriptionError(
                description,
