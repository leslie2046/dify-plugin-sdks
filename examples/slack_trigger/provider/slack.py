...
class SlackTrigger(Trigger):
    ...
    def _dispatch_event(
        ...
        payload, response = self._parse_request(
            signing_secret=signing_secret, request=request
        )
        dispatch_event = self._determine_event(
    ...
    def _parse_request(
        self, signing_secret: str, request: Request
