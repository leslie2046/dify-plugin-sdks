...
# CASE 2: Tool call sequences where IDs are anchored to the first chunk
# (vLLM/SiliconFlow ...)
INPUTS_CASE_2 = [
    ToolCall(
        id="1",
...
def test__increase_tool_call():
    ...
    mock_id_generator.side_effect = [exp_case.id for exp_case in EXPECTED_CASE_4]
    with patch(
        "dify_plugin.interfaces.model.openai_compatible.llm._gen_tool_call_id",
        mock_id_generator,
    ):
