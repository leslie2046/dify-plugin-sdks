...
def test_construct_website_crawl_datasource():
        ...
        def _get_website_crawl(
            ...
            yield DatasourceMessage(
                ...
                message=DatasourceMessage.TextMessage(
                    text=(
                        "Website crawl result for "
                        f"{datasource_parameters.get('url', 'unknown')}"
                    )
                ),
            )
    ...
    datasource = Website(
        runtime=DatasourceRuntime(credentials={}, user_id="test", session_id="test"),
        session=session,
    )
    ...
    class OnlineDocument(OnlineDocumentDatasource):
        ...
        def _get_content(
            ...
            yield DatasourceMessage(
                ...
                message=DatasourceMessage.TextMessage(
                    text=(
                        "Online document page content for "
                        f"{page.workspace_id} - {page.page_id}"
                    )
                ),
            )
